...
| Task | Files                                                | Description                              |
| ---- | ---------------------------------------------------- | ---------------------------------------- |
| 1    | `src/handling/`, `src/algorithm/`, `src/monitoring/` | **3-Pillar Architecture** Implementation |
| 2    | `Makefile`, `scripts/`                               | Build system modernization               |
| 3    | `configure.ac`, `Makefile.am`                        | Removal of Autotools build system        |

### Task 1: 3-Pillar Architecture Implementation

The monolithic `state.c` has been decomposed into focused modules following the **3-Pillar Architecture**:

- **Algorithm Pillar** (`src/algorithm/`):

  - Implemented predictive models: `markov.c` and `vomm.c`.
  - Isolated prediction logic from state management.

- **Handling Pillar** (`src/handling/`):

  - `state_io.c`: Manages `.state` reading/writing.
  - `exe.c`, `map.c`: Handle execution and memory mapping.
  - `context.c`: Application context management.

- **Monitoring Pillar** (`src/monitoring/`):

  - `spy.c`: Process monitoring.
  - `proc.c`: `/proc` interactions.

- `daemon/`, `config/`, `utils/`: Core logic, configuration, and helpers.

This modular approach ensures separation of concerns, maintainability, and testability.
...
| Task | Files      | Description              |
| ---- | ---------- | ------------------------ |
| 1    | `tests/`   | New Unit Test Suite      |
| 2    | `Makefile` | `make test` target added |

### Task 1: New Unit Test Suite

Added a comprehensive `tests/` directory to verify core logic:

- `test_markov.c`: Verifies Markov chain logic.
- `test_vomm.c`: Verifies VOMM prediction accuracy.
- `test_state_io.c`: Validates state persistence and format compatibility.

### Task 2: Makefile Verification

Added `make test` target. The test suite passes all unit tests and `state_io.c` has been validated to correctly read legacy `.state` files, ensuring 100% backward compatibility.
